...
export type GameConfirmation = Readonly<{
  title: string; description: string; amount?: bigint; busy?: boolean; error?: string;
...
export function GameFrame({ children, friends, selectedFriendId, onSelectFriend, friendsLoading, friendsError, onConnect, wallet, confirmation, connection, mode, onMenuChange }: GameFrameProps) {
  const [menu, setMenu] = useState<"friends" | "wallet" | null>(null);
  const friend = friends.find(value => value.id === selectedFriendId);
  const selecting = !friend || menu === "friends";
  const menuOpen = selecting || menu !== null || Boolean(confirmation);
  ...
  return <section className="rf-game-frame" aria-label="Game container" data-mode={mode} style={{ aspectRatio: `${GAME_VIEWPORT.width} / ${GAME_VIEWPORT.height}` }}>
    <div className="rf-frame-chrome" inert={menuOpen || undefined}>
      <div className="rf-frame-toolbar">
        <span className="rf-frame-mode">{mode === "preview" ? "Local preview" : "Rare Friends"}</span>
    ...
    {confirmation ? <GameMenu title={confirmation.title} onClose={confirmation.busy ? undefined : confirmation.onCancel}
      ...
      <p>{mode === "preview" ? friends.some(value => value.kind === "owned") ? "Choose your Friend for this local preview. RF, bait and catches are simulated." : "Choose a sample Friend. Each has separate simulated RF, bait and catches." : "Choose an owned, hardwired Generations NFT. Its inventory and RF stay with its wallet."}</p>
      {connection}
      ...
      <h3>{friend?.label}</h3><p>{mode === "preview" ? "Preview balance. RF is simulated and no transactions are sent." : "Bait, catches and RF belong to this Friend’s canonical wallet."}</p>
      ...
      <button type="button" onClick={() => setMenu("friends")}>Change Friend</button>
    </GameMenu> : null}
  </section>;
}
